...
var newMarker = new mapboxgl.Marker({color: 'rebeccapurple', draggable: true})
...
var alamoPopup = new mapboxgl.Popup({closeOnClick: true, closeOnMove: true})
    .setHTML("<h2>Remember Me!</h2>")
    .addTo(map)
...
geocode("16585 Blanco Rd, San Antonio, TX", mapboxApiKey).then(function(results){
    console.log(results);
});
...
geocode("600 Navarro St #600, San Antonio, TX", mapboxApiKey).then(function(results){
...
reverseGeocode({lat: 29.4260, lng:-98.461}, mapboxApiKey).then(function(results){
    console.log(results);
})
